...
function reportFillError(f::ROCArray{T,D},u,u⁰,δt,d,tol) where {T,D}
    ...
    if maxf-1 > tol
        AMDGPU.@allowscalar du⁰,du = abs(div(maxid,u⁰)),abs(div(maxid,u))
        @printf("|∇⋅u⁰| = %+13.8f, |∇⋅u| = %+13.8f\n",du⁰,du)
        for d∈1:D
            AMDGPU.@allowscalar @printf("    %d -- uLeftδt: %+13.8f, uRightδt: %+13.8f\n", d, (u[maxid,d]+u⁰[maxid,d])*δt, (u[maxid+δ(d,maxid),d]+u⁰[maxid+δ(d,maxid),d])*δt)
        end
    ...
    if minf < -tol
        AMDGPU.@allowscalar du⁰,du = abs(div(minid,u⁰)),abs(div(minid,u))
        @printf("|∇⋅u⁰| = %+13.8f, |∇⋅u| = %+13.8f\n",du⁰,du)
        for d∈1:D
